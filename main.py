...
from src import primary_person_df, units_df, damages_df, charges_df, spark
...
def main():
    # Q1:
    cnt_death = males_killed_greater_than_2(primary_person_df)
    print(f'Number of males death greater than 2: {cnt_death}')

    # Q2:
    two_wheeler_crash = two_wheeler_count(units_df)
    ...
    top_5_car_crash(primary_person_df, units_df).show()

    # Q4
    licence_cnt = valid_driver_license_count(primary_person_df, units_df)
    ...
    state_with_highest_accidents_no_females(primary_person_df).show()

    # Q6
    most_injuries = vehicle_models_with_most_injurie(primary_person_df, units_df)
    ...
    top_ethnic_group_body_style(primary_person_df, units_df).show()

    # Q8:
    print('Top 5 Zip Codes with highest number crashes with '
          'alcohols as the contributing factor to a crash:')
    crash_due_to_alcohol_by_zip_code(primary_person_df).show()
    ...
    print(f'Count of Distinct Crash IDs where No Damaged Property was observed '
          ...
          f' {crash_id_with_no_property_damage(damages_df, units_df)}')
    ...
    speeding_offence(primary_person_df, units_df, charges_df).show()
